...
con = sqlite3.connect('tutoring_school.sqlite')
cur = con.cursor()

# Выполнение скрипта для создания таблиц и заполнения данными
con.executescript('''
...
df2 = pd.read_sql('SELECT * FROM teacher;', con)
df3 = pd.read_sql('SELECT * FROM subject;', con)
df4 = pd.read_sql('SELECT * FROM program;', con)
df5 = pd.read_sql('SELECT * FROM schedule;', con)
df6 = pd.read_sql('SELECT * FROM registration;', con)
